...
try:
    ...
    from utils_filter import OneEuroFilter
...
app.config['SECRET_KEY'] = 'advanced_demo'
...
def gaze_tracking_loop():
    ...
    print("Starting Advanced Eye Tracking Loop...")
    ...
    last_point_change_time = time.time()
    CALIBRATION_TIMEOUT = 3.0  # Seconds to wait before forcing next point
    
    # Filters for smoothing
    filter_x = OneEuroFilter(time.time(), 0.5, min_cutoff=0.1, beta=0.05)
    filter_y = OneEuroFilter(time.time(), 0.5, min_cutoff=0.1, beta=0.05)
    ...
    while gaze_running:
        ...
        try:
            ...
            if calibrate_mode:
                # Check for timeout
                if current_time - last_point_change_time > CALIBRATION_TIMEOUT:
                    print(f"Timeout! Forcing move to next point ({iterator+1}/{n_points})")
                    gestures.clb["web_gaze"].movePoint()
                    # Manually advance iterator since we forced it
                    # Note: gestures.step might not have returned a changed point yet
                    # but movePoint() updates the internal matrix index.
                    # We need to sync our iterator.
                ...
                if cevent and cevent.point is not None:
                    ...
                    if cevent.point[0] != prev_x or cevent.point[1] != prev_y:
                        ...
                        last_point_change_time = current_time
                ...
                raw_x = event.point[0] / VIRTUAL_W
                raw_y = event.point[1] / VIRTUAL_H
                
                # Apply OneEuroFilter
                smooth_x = filter_x.filter(current_time, raw_x)
                smooth_y = filter_y.filter(current_time, raw_y)
...
@app.route('/')
def index():
    # Client-side tracking (WebGazer) so the browser asks for camera permission
    return render_template('gaze_demo.html')

# Optional: keep the server-driven version at /server-gaze
@app.route('/server-gaze')
def server_gaze():
    return render_template('gaze_ui.html')
...
if __name__ == '__main__':
    print("Starting Advanced EyeGestures Web Demo...")
